...
import { ShoppingCart, ShieldCheck, Warehouse, RefreshCw, Truck, Star, Send, Loader2, MessageSquare, AlertCircle, CheckCircle2, Check, Sparkles, Zap } from 'lucide-react';
...
export const ProductQuickViewModal: React.FC<ProductQuickViewModalProps> = React.memo(({
  ...
  const { activeSession } = useGroupShopping();
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-4 border-b border-slate-200 pb-2">
          <button
            ...
            className={`text-xs sm:text-sm font-extrabold pb-1 border-b-2 transition-all flex items-center gap-1.5 ${
              activeTab === 'details'
        ...
        {activeTab === 'details' && (
          <div className="grid grid-cols-1 md:grid-cols-12 gap-5 pt-1 items-start">
            {/* Image & Gallery Side (5 cols) */}
            <div className="md:col-span-5 space-y-2.5">
              <div className="relative h-56 sm:h-64 w-full rounded-xl overflow-hidden bg-slate-100 border border-slate-200 shadow-xs">
                <Image
                ...
                <div className="absolute bottom-2.5 left-2.5 z-10">
                  <Badge variant="info" size="sm" className="bg-white/90 backdrop-blur-md text-slate-800 border-slate-200 shadow-xs font-semibold text-[10px] py-0.5 px-2">
            ...
            {/* Details & Variant Selection Side (7 cols) */}
            <div className="md:col-span-7 flex flex-col justify-between space-y-3 h-full">
              <div className="space-y-3">
                <div>
                  <div className="flex items-center justify-between text-xs text-slate-500 mb-1">
                    <span className="font-bold text-indigo-600 text-[11px]">{product.vendor?.name || 'Vendor Partner'}</span>
                  ...
                  <h2 className="text-lg font-extrabold text-slate-900 mb-1 leading-snug">{product.title}</h2>
                  <p className="text-xs text-slate-600 leading-relaxed font-medium line-clamp-2">{product.description}</p>
                </div>
                  ...
                  <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200/90 space-y-2">
                    ...
                    <div className="space-y-2">
                      {Object.entries(product.attributes).map(([attrKey, attrVal]) => {
                        ...
                        return (
                          <div key={attrKey} className="flex items-center justify-between text-[11px] py-0.5 border-b border-slate-200/60 last:border-none">
                ...
                <div className="p-2 rounded-lg bg-purple-50/80 border border-purple-200/80">
                  ...
                  <p className="text-[10px] text-purple-800 mt-0.5 font-medium leading-tight">
                    Fulfilling from{' '}
                ...
                {/* Guarantees */}
                <div className="space-y-1 text-[11px] text-slate-500 border-t border-slate-100 pt-2 font-medium">
                ...
                <div className="flex items-center gap-2">
                  {activeSession && onSuggestToGroup && (
                    <Button
                      ...
                      className="border-indigo-200 text-indigo-700 bg-indigo-50 hover:bg-indigo-100 font-semibold text-xs py-1.5"
                    >
                      Suggest
                    </Button>
                  )}
                  ...
                  <Button
                    ...
                    className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold shadow-md shadow-indigo-600/25 text-xs py-2 px-4"
                  >
                    Add to Cart
                  </Button>
                  ...
                  {onBuyNow && (
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-extrabold shadow-md shadow-amber-500/25 text-xs transition-colors flex items-center gap-1.5 cursor-pointer"
                    >
