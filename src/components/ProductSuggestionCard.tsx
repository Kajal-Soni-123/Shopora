...
export function ProductSuggestionCard({ message }: ProductSuggestionCardProps) {
  const { activeMember, toggleVote, addItemToGroup, isItemInGroupCart, activeSession } = useGroupShopping();
  ...
  return (
    <div className="bg-white border border-indigo-100 rounded-2xl p-3.5 shadow-sm hover:shadow-md transition-all my-2 space-y-3 font-sans">
      ...
      <div className="flex items-center justify-between text-xs border-b border-slate-100 pb-2">
        <span className="inline-flex items-center gap-1.5 font-bold text-indigo-700 bg-indigo-50 px-2.5 py-0.5 rounded-full text-[11px]">
      ...
      {/* Add to Group Cart Button */}
      {isItemInGroupCart(product.id) ? (
        <div className="w-full py-2.5 px-3 rounded-xl text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center justify-center gap-2">
          <svg className="w-4 h-4 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
          </svg>
          <span>In Group Cart</span>
        </div>
      ) : (
        <button
        ...
        >
          {addedSuccess ? (
            ...
            <>
              ...
              {isAdding ? 'Adding...' : 'Add to Group Cart'}
            </>
          )}
        </button>
      )}
